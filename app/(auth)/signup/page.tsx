...
export default function SignupPage(): JSX.Element {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    // Connection required using better auth
    ...
    <div className="min-h-screen bg-[#0A0B0F] flex flex-col items-center justify-center px-4 relative overflow-hidden">
      ...
      >
        <div className="rounded-2xl border border-white/8 bg-[#111216] shadow-2xl shadow-black/60 p-7">
          ...
          <div className="mb-7">
            ...
            <p className="text-xs text-white/35">
              Start understanding your invoices — free forever
            </p>
          </div>
          ...
          <AnimatePresence mode="wait">
            {done ? (
              ...
              >
                ...
                <p className="text-white/35 text-xs">Check your email to verify your account.</p>
              </motion.div>
            ) : (
              ...
              >
                {fields.map((field) => (
                  <FloatingInput
                    key={field.id}
                    field={field}
                    value={form[field.id as keyof SignupForm]}
                    onChange={handleChange}
                  />
                ))}

                <p className="text-[11px] text-white/20 pt-0.5 leading-relaxed">
                  By signing up you agree to our{" "}
                  <Link href="/terms" className="text-white/40 hover:text-amber-400 transition-colors underline underline-offset-2">
                    Terms
                  </Link>{" "}
                  and{" "}
                  <Link href="/privacy" className="text-white/40 hover:text-amber-400 transition-colors underline underline-offset-2">
                    Privacy Policy
                  </Link>
                  .
                </p>
        ...
        <p className="text-center text-[11px] text-white/20 mt-5">
          ...
          <Link href="/signin" className="text-amber-400/70 hover:text-amber-400 transition-colors">
            Sign in
          </Link>
        </p>
      </motion.div>
